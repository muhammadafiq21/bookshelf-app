...
window.addEventListener('load', () => {
  ...
  $('#isComplete').onclick = (e) => {
    $('form>button').innerText = e.target.checked ? 'Masukkan Buku ke rak Selesai Dibaca' : 'Masukkan Buku ke rak Belum Selesai Dibaca';
  };
...
function updateUI(books) {
  ...
  for (let book of books) {
    ...
    let group = $new('button', { css: ['button', 'button-success'], text: `${book.isComplete ? 'Belum selesai' : 'Selesai dibaca'}`, action: moveGroup });
    let remove = $new('button', { css: ['button', 'button-danger'], text: 'Hapus', action: hapusBuku });
    let edit = $new('button', { css: ['button', 'button-warning'], text: 'Edit', action: editBuku });
